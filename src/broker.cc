...
void *Broker::Client(void *arg) {
  ...
  char banner[128];
  char *protocol = strdup(PACKAGE_VERSION);
  char *period = strchr(protocol, '.');
  if (!period)
    Error(110, "There is no '.' in PACKAGE_VESION, contact developer please");
  *period = 0;
  ...
  if (Send(parg->client_sockfd_, banner, strlen(banner))) {
    ...
    for (;;) {
      ...
      char *annotate_ptr = parg->casefolding_->FoldOrDie(input_buf+3,
